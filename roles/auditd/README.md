...
- Deploys `/etc/audit/auditd.conf` with compliance-sized buffers,
  log rotation, and disk-space failure modes
...
- Makes the rule set immutable (`-e 2`) by default — CIS-recommended
  but locks rule changes until reboot (see defaults rationale)
