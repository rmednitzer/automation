...
Base system management and hardening for Ubuntu 24.04 LTS (Noble
Numbat).
...
- Applies a sysctl baseline (network, VM, file-handles, hardlink /
  ...
  per GDPR Art 5(1)(c))
- Hardens the kernel: blacklists unused filesystem and network
  modules, disables core dumps, enables ASLR, restricts kernel
  pointer / dmesg / unprivileged BPF / unprivileged user namespaces /
  `ptrace_scope`
- Hardens the filesystem: writes a `tmpfs` fstab entry for `/tmp`
  with `noexec,nosuid,nodev`, bind-mounts `/var/tmp` to `/tmp`, sets
  the sticky bit on any world-writable directories found, and
  re-asserts permissions on `/etc/{shadow,gshadow,passwd,group,crontab}`
