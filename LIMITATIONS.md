...
| L2 | First Molecule scenario landed for the `users` role (`roles/users/molecule/default/`); remaining roles still uncovered. CI gains a `molecule` matrix job and `make molecule`/`make molecule-deps` targets. **The scenario has not been executed in this environment (no Docker); it is authored and YAML/lint-validated only.** | Run `make molecule` in a Docker-enabled CI/host; extend the matrix to `ssh_hardening`, `auditd`, and the rest. |
